...
@section('title', 'Create New Course - Course System')

@section('content')
<!-- Create Course UI: consistent with other pages (Poppins, blue bg, wheels, waves, bold headings) -->
<style>
    ...
    @media (prefers-reduced-motion: reduce) {
        ...
        .btn-wave, .btn-secondary { transition: none !important; }
    }
</style>
...
<div class="page-wrap container">
    <div class="page-header">
        <div>
            ...
            <small class="muted-small">Add a new course to the system — fields marked required must be completed.</small>
        </div>
        <div class="text-end">
            <small class="muted-small d-block">Signed in as</small>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="prerequisites" name="prerequisites[]" multiple>
                                @foreach($courses as $prereq)
                                    <option value="{{ $prereq->id }}" @if(collect(old('prerequisites'))->contains($prereq->id)) selected @endif>{{ $prereq->code }} - {{ $prereq->name }}</option>
                                @endforeach
                            </select>
...
<script>
    ...
    // keyboard accessibility for submit buttons
