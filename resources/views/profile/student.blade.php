...
@section('content')
<!-- Student Profile UI adapted from Dashboard style:
     - Poppins font
     - bright light-blue background
     - rotating wheels
     - subtle wave ripple on action buttons (data-wave)
     - bold headings in Poppins black
     - tables with thick black outline for strong contrast
-->
<style>
    ...
    /* bold headings requested */
    .menu-title, .enroll-title, .profile-title { font-family: 'Poppins', inherit; font-weight: 700; color: #000; }

    /* profile details */
    .profile-grid { display: grid; grid-template-columns: 1fr 340px; gap: 1.25rem; align-items: start; }
    ...
    /* button (same wave trigger behaviour) */
    ...
    /* THICK BLACK OUTLINE on table wrappers for strong contrast as requested */
    .table-responsive {
        border-radius: 10px;
        overflow: hidden;
        outline: 6px solid rgba(0,0,0,1); /* thick black */
        border: 3px solid rgba(0,0,0,1); /* backup border */
        box-shadow: 0 10px 30px rgba(0,0,0,0.06);
        background: white;
    }

    /* improve readability on bright bg */
        ...
        <div class="col-12">
            <div class="page-header">
                <div>
                    ...
                    <small class="text-muted">Personal details and current enrollments</small>
                </div>
                <div class="text-end">
            ...
            <div class="profile-grid">
                ...
                <div class="profile-main">
                    <div class="card system-card">
                        ...
                        <div class="card-body">
                            <h5 class="card-title" style="font-weight:700; margin-bottom:.5rem;">{{ Auth::user()->student->student_name }}</h5>
                            <p class="profile-item"><strong>Email:</strong> {{ Auth::user()->email }}</p>
                            <p class="profile-item"><strong>Phone:</strong> {{ Auth::user()->student->phone ?? '-' }}</p>
                            <p class="profile-item"><strong>Date of Birth:</strong> {{ Auth::user()->student->date_of_birth ?? '-' }}</p>
                            <p class="profile-item"><strong>Address:</strong> {{ Auth::user()->student->address ?? '-' }}</p>
                            <p class="profile-item"><strong>Enrolled Courses:</strong> {{ Auth::user()->student->enrolledCourses()->count() }}</p>
                            ...
                            <div class="d-flex gap-2" style="margin-top:1rem;">
                                ...
                                <a href="{{ route('enrollments.available') }}" class="btn-outline" role="button" data-wave>Browse Courses</a>
                            </div>
                        </div>
                    </div>

                    <!-- Enrollments preview (table) -->
                            ...
                            @if(Auth::user()->student->enrolledCourses()->count() > 0)
                                <div class="table-responsive">
                                    <table class="table table-hover align-middle">
                                        ...
                                        <tbody>
                                            @foreach(Auth::user()->student->enrolledCourses()->take(8)->get() as $course)
                                            <tr>
                                                <td>{{ $course->name }}</td>
                                                <td>{{ $course->code }}</td>
                                                <td>{{ $course->credits }}</td>
                                                <td><span class="badge bg-primary text-white" style="font-weight:600;">Enrolled</span></td>
                                            </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                                ...
                                <div class="text-center mt-3">
                                    <a href="{{ route('enrollments.my-courses') }}" class="btn btn-primary" data-wave>View All My Courses</a>
                                </div>
                            @else
                    ...
                    <div class="card system-card system-info-card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-grid gap-2">
                                ...
                                <a href="{{ route('enrollments.available') }}" class="btn btn-outline" data-wave>Browse Courses</a>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    // periodic gentle waves at random positions removed from UI here to reduce noise.
    // wave on button hover/click (data-wave) - kept
